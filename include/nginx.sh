...
Install_Nginx()
{
    ...
    ./configure --user=www --group=www --prefix=/usr/local/nginx --with-http_stub_status_module --with-http_ssl_module --with-http_gzip_static_module --with-ipv6 --with-http_sub_module ${NginxMAOpt}
    make && make install
    cd ../
    ...
    \cp conf/rewrite/moodle.conf /usr/local/nginx/conf/moodle.conf
